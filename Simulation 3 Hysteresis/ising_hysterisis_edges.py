...
# global variables
grid = numpy.random.random_integers(0, 1, (200, 200)) # Initialize grid with random 1s and 0s
...
def ising(N, T, B):
	# Initialization
	n_iterations = 500000 # How many times the step is done
...
def metropolis(T, n, B1):
	N = 50
	n_iterations = n
	B = 0 # From -20 to +20
	step = numpy.float32(B1)/(numpy.float32(n)/5.0)
	...
	print ('hysteresis,N='+str(N)+',n='+str(n)+',t='+str(T)+'.png')
	print step
	print ('hysteresis,N='+str(N)+',n='+str(n)+',t='+str(T)+'.h5')
	...
	B = (int(n/5))*step - (168 - int(n/5))*step
	for i in range(N): # Make 0s to -1s
		for j in range(N):
			grid[i, j] = 1
	...
	B = (int(n/5))*step - (int((3*n)/5) - int(n/5))*step + (280 - int((3*n)/5))*step;
	for i in range(N): # Make 0s to -1s
		for j in range(N):
			grid[i, j] = -1
	...
	h5f = h5py.File('edges_hysteresis,N='+str(N)+',n='+str(n)+',t='+str(T)+'.h5', 'w')
