...
def metropolis(N, T, n, B1, trial):
	...
	print ('data/Periodic-boundaries/N='+str(N)+'/Trial '+str(trial)+'/hysteresis,N='+str(N)+',n='+str(n)+',t='+str(T)+',j='+str(J)+'.png')
	print ('data/Periodic-boundaries/N='+str(N)+'/Trial '+str(trial)+'/hysteresis,N='+str(N)+',n='+str(n)+',t='+str(T)+',j='+str(J)+'.h5')
	...
	i = 0
	Ms[i] = calculate_magnetization(N)/numpy.float32(N*N)
	Es[i] = calculate_energy(N, B)/numpy.float32(N*N)
	Ts[i] = T
	Bs[i] = B
	plt.scatter(B, Ms[i])
	B += step
	...
	plt.savefig('data/Periodic-boundaries/N='+str(N)+'/Trial '+str(trial)+'/hysteresis,N='+str(N)+',n='+str(n)+',t='+str(T)+',j='+str(J)+'.png')
	...
	h5f = h5py.File('data/Periodic-boundaries/N='+str(N)+'/Trial '+str(trial)+'/hysteresis,N='+str(N)+',n='+str(n)+',t='+str(T)+',j='+str(J)+'.h5', 'w')
...
grid = numpy.random.random_integers(0, 1, (75, 75))
# im = plt.imshow(grid, cmap='gray', interpolation="nearest")	

metropolis(75, 2.3, 50, 0.25, 4)
# metropolis(10, 1.5, 100, 0.6, 1)

# for i in range(1,3):
# 		  # N,   t,   n,   J, trial
# 	metropolis(50, 1.2, 100, 0.6, i+1)
# 	metropolis(50, 1.3, 100, 0.6, i+1)
# 	metropolis(50, 1.4, 100, 0.6, i+1)
# 	metropolis(50, 1.5, 100, 0.6, i+1)
